...
class ApplicationWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.video_player.positionChanged.connect(self.positionChanged) # Update slider based on video position
        self.video_player.durationChanged.connect(self.durationChanged) # Slider length based on video duration 
    ...
    def callback_load(self):
        ...
        file = QtCore.QDir.current().filePath(file_array[row]) # Take that row and load the file 
...
#        self.ui.video_widget_input.setAspectRatioMode(QtCore.Qt.KeepAspectRatioByExpanding)
    ...
    def positionChanged(self):
        ...
        self.frame_number = round((self.video_player.position()/1000)*24)
        print("Frame Number: " + str(self.frame_number)) # DEBUGGING 
        
        # Get the current video time
        duration = str(datetime.timedelta(seconds=round(self.video_player.duration()/1000)))
        current_pos = str(datetime.timedelta(seconds=round(self.video_player.position()/1000)))
        ...
        self.ui.horizontalSlider.setValue(self.video_player.position())

    def durationChanged(self):
        # Update the slider proportions  
        self.ui.horizontalSlider.setRange(0, self.video_player.duration())
    ...
    def update_csv(self):
        ...
        data_array = self.csvFileArray[10,1].split(' ') # CAHNGE 10 to self.frame_number-1 (10 gets frame 9)
